...
int main(int argc, char** argv) {
    ...
    */
    ...
    iface->write(address++, nvm::Instruction::FixedLiteralJumpNegative);//JFLN loopStart //loop if not less than 10
    iface->write<nvm::address_t>(address++, loopStart);                 //

    while (core.getInstructionPointer() < address) {
        std::cout << "address: " << core.getInstructionPointer() << std::endl;
        core.process();
    }
