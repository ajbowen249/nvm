...
#define INCDEC_TEST(INST, REGCAM, REGLOW, REGTYPE)                                     \
typedef std::tuple<REGTYPE, REGTYPE, nvm::test::ExpectFlags> INST ## REGCAM ## Params; \
class INST ## REGCAM :                                                                 \
    public nvm::test::BasicTestFixture,                                                \
    public ::testing::WithParamInterface<INST ## REGCAM  ## Params> { };               \
                                                                                       \
TEST_P(INST ## REGCAM, INST ## REGCAM) {                                               \
    ...
    iface_->write<uint8_t>(address++, (nvm::RegisterType::REGLOW << 4) | 0x00);        \
                                                                                       \
    processIterations(2);                                                              \
    auto result = core_.get ## REGCAM ## Register(0);                                  \
                                                                                       \
    EXPECT_EQ(std::get<1>(param), result);                                             \
    std::get<2>(param).verify(core_);                                                  \
}                                                                                      \

#define INCDEC_ALL(INST)                \
INCDEC_TEST(INST, Ui8, ui8, uint8_t)    \
INCDEC_TEST(INST, I8, i8, int8_t)       \
INCDEC_TEST(INST, Ui16, ui16, uint16_t) \
INCDEC_TEST(INST, I16, i16, int16_t)    \
INCDEC_TEST(INST, Ui32, ui32, uint32_t) \
INCDEC_TEST(INST, I32, i32, int32_t)    \
INCDEC_TEST(INST, F32, f32, f32_t)      \
INCDEC_TEST(INST, F64, f64, f64_t)      \
